...
function pruneInPlace(room) {
  ...
  // If the OWNER was pruned, they've been silent for the full stale window
  // (STALE_MS) — a genuine drop, not a refresh (which reconnects in seconds).
  // Release the owner anchor so the host role hands off PERMANENTLY to a
  // remaining player: a returning ex-owner should not yank host back after
  // being gone this long. (A brief refresh never reaches here — it's not
  // pruned — so a host who reloads still keeps the role.)
  if (room.ownerId && !room.players[room.ownerId]) {
    room.ownerId = null;
    changed = true;
  }

  // resolveHost promotes a present player to host whenever the current host is
  // missing, so a dropped host is always replaced by someone still in the room.
